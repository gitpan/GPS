...
sub parse_serial_data {
	my $sentances = $gps->get_sentances();
...
sub select_port {
	...
	if ($port ne $gps->{serialport}) {
		if ($gps->{serial}) {
			$gps->{serial}->close || die "failed to close serialport";
			undef $gps->{serial}; # frees memory back to perl
		}
...
sub build_menubar {
	...
	my $ver_dialog =   $mw->Dialog(
		-title   => 'TKB_GPS Version',
		-text    => "TKB_GPS\n\nVersion $VERSION",
		-buttons => ['OK'],
		-bitmap  => 'info');
	my $about_dialog = $mw->Dialog(
		-title   => 'About TKB_GPS',
		-text    => 'TKB_GPS was built by Travis Kent Beste.  He can be reached at travis@tencorners.com',
		-buttons => ['OK']);
